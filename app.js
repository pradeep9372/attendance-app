(function () {
  "use strict";

  const employeeIdInput = document.getElementById("employeeId");
  const employeeNote = document.getElementById("employeeNote");
  const toggleBtns = Array.from(document.querySelectorAll(".toggle-btn"));
  const locateBtn = document.getElementById("locateBtn");
  const locationNote = document.getElementById("locationNote");
  const pinField = document.getElementById("pinField");
  const employeePinInput = document.getElementById("employeePin");
  const pinNote = document.getElementById("pinNote");
  const submitBtn = document.getElementById("submitBtn");
  const backBtn = document.getElementById("backBtn");
  const resultBox = document.getElementById("result");
  const statusDot = document.getElementById("statusDot");
  const statusText = document.getElementById("statusText");
  const offlineBanner = document.getElementById("offlineBanner");
  const clockLine = document.getElementById("clockLine");
  const installBtn = document.getElementById("installBtn");

  let state = {
    type: "IN",
    coords: null,
    employeeVerified: false,
    awaitingPin: false, // false = filling details, true = PIN step
  };

  // ---------------------------------------------------
  // Device ID — a random ID generated once per phone and
  // saved in localStorage. Sent with every submit so the
  // backend can lock an Employee ID to the first device
  // that successfully uses it (see Code.gs).
  // ---------------------------------------------------
  function getDeviceId() {
    const STORAGE_KEY = "attendance_device_id";
    let id = localStorage.getItem(STORAGE_KEY);
    if (!id) {
      id = (crypto.randomUUID && crypto.randomUUID()) ||
        "dev-" + Date.now() + "-" + Math.random().toString(16).slice(2);
      localStorage.setItem(STORAGE_KEY, id);
    }
    return id;
  }
  const deviceId = getDeviceId();

  // A short, human-readable summary of the device (for the
  // Employees sheet's "Device Info" column) — separate from
  // the deviceId above, which is just an opaque matching code.
  function getDeviceDescription() {
    const ua = navigator.userAgent || "";
    let os = "Unknown OS";
    if (/Android\s([\d.]+)/.test(ua)) os = "Android " + RegExp.$1;
    else if (/iPhone OS\s([\d_]+)/.test(ua)) os = "iOS " + RegExp.$1.replace(/_/g, ".");
    else if (/Windows NT/.test(ua)) os = "Windows";
    else if (/Mac OS X/.test(ua)) os = "macOS";

    let browser = "Unknown browser";
    if (/Edg\//.test(ua)) browser = "Edge";
    else if (/Chrome\/([\d.]+)/.test(ua)) browser = "Chrome " + RegExp.$1.split(".")[0];
    else if (/Firefox\/([\d.]+)/.test(ua)) browser = "Firefox " + RegExp.$1.split(".")[0];
    else if (/Safari\//.test(ua)) browser = "Safari";

    return os + " · " + browser;
  }
  let deviceInfo = getDeviceDescription();

  // Best-effort: on Android + Chromium-based browsers, this newer API
  // can reveal the actual device model (e.g. "SM-A536E"). It doesn't
  // exist on iOS/Safari or older browsers — deviceInfo just stays as
  // the OS/browser summary above in that case, which is fine.
  if (navigator.userAgentData && navigator.userAgentData.getHighEntropyValues) {
    navigator.userAgentData
      .getHighEntropyValues(["model"])
      .then((info) => {
        if (info && info.model) {
          deviceInfo = deviceInfo + " · " + info.model;
        }
      })
      .catch(() => {
        // Unsupported or blocked — keep the fallback description.
      });
  }

  // ---------------------------------------------------
  // Live clock line
  // ---------------------------------------------------
  function tickClock() {
    const now = new Date();
    clockLine.textContent = now.toLocaleString(undefined, {
      weekday: "short",
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit",
    });
  }
  tickClock();
  setInterval(tickClock, 1000);

  // ---------------------------------------------------
  // Online / offline status
  // ---------------------------------------------------
  function updateOnlineStatus() {
    const online = navigator.onLine;
    statusDot.classList.toggle("is-online", online);
    statusDot.classList.toggle("is-offline", !online);
    statusText.textContent = online ? "Online" : "Offline";
    offlineBanner.classList.toggle("visible", !online);
    evaluateSubmitEnabled();
  }
  window.addEventListener("online", updateOnlineStatus);
  window.addEventListener("offline", updateOnlineStatus);
  updateOnlineStatus();

  // Wake up the Apps Script backend early so Submit feels faster later
  apiRequest({ action: "ping" }).catch(() => {});

  // ---------------------------------------------------
  // Backend request helper — uses fetch() with CORS.
  //
  // NOTE: this app previously used a <script src=...> JSONP
  // technique, because Apps Script Web Apps historically
  // didn't support fetch()/CORS reliably. Google has since
  // started blocking that script-tag technique (it now fails
  // with a network error regardless of network/device), while
  // fetch() with mode:"cors" works correctly. So we call the
  // backend directly with fetch() and no `callback` param —
  // the backend already returns plain JSON when no callback
  // is present (see the jsonp() function in Code.gs).
  // ---------------------------------------------------
  async function apiRequest(params, timeoutMs, isRetry) {
    if (!CONFIG.API_URL || CONFIG.API_URL.indexOf("PASTE_YOUR_WEB_APP_URL_HERE") !== -1) {
      throw new Error("The app isn't connected to a backend yet. Add your Web App URL in config.js.");
    }

    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs || 25000);

    try {
      const query = new URLSearchParams({ api: "attendance", ...params });
      const res = await fetch(CONFIG.API_URL + "?" + query.toString(), {
        method: "GET",
        mode: "cors",
        signal: controller.signal,
      });

      clearTimeout(timer);

      if (!res.ok) {
        throw new Error("Server returned an error (status " + res.status + ").");
      }

      return await res.json();
    } catch (err) {
      clearTimeout(timer);

      // Retry once — covers both a slow Apps Script "wake up"
      // (abort/timeout) and a brief mobile network drop.
      if (!isRetry) {
        await new Promise((resolve) => setTimeout(resolve, 800));
        return apiRequest(params, timeoutMs, true);
      }

      if (err.name === "AbortError") {
        throw new Error("The request timed out. Check your connection and try again.");
      }

      throw new Error("Couldn't reach the backend. Check the Web App URL and deployment settings.");
    }
  }

  // ---------------------------------------------------
  // Employee ID lookup (on blur, debounced-ish)
  // ---------------------------------------------------
  let lookupToken = 0;
  employeeIdInput.addEventListener("blur", async () => {
    const id = employeeIdInput.value.trim();
    state.employeeVerified = false;
    evaluateSubmitEnabled();

    if (!id) {
      setNote(employeeNote, "", null);
      return;
    }

    const myToken = ++lookupToken;
    setNote(employeeNote, "Checking employee ID…", null);

    try {
      const data = await apiRequest({ action: "employee", employeeId: id });
      if (myToken !== lookupToken) return; // stale response

      if (data && data.success) {
        state.employeeVerified = true;
        localStorage.setItem("attendance_employee_id", id); // remember for next time
        setNote(employeeNote, `${data.employeeName}${data.department ? " · " + data.department : ""}`, "good");
      } else {
        setNote(employeeNote, (data && data.message) || "Employee ID not found.", "bad");
      }
    } catch (err) {
      if (myToken !== lookupToken) return;
      setNote(employeeNote, err.message, "bad");
    }
    evaluateSubmitEnabled();
  });

  function setNote(el, text, kind) {
    el.textContent = text || "\u00A0";
    el.classList.toggle("is-good", kind === "good");
    el.classList.toggle("is-bad", kind === "bad");
  }

  // ---------------------------------------------------
  // Remember Employee ID across visits (safe to remember —
  // it's not a secret, unlike the PIN, which is never saved)
  // ---------------------------------------------------
  const savedEmployeeId = localStorage.getItem("attendance_employee_id");
  if (savedEmployeeId) {
    employeeIdInput.value = savedEmployeeId;
    employeeIdInput.dispatchEvent(new Event("blur")); // triggers the same lookup as a manual blur
  }

  // ---------------------------------------------------
  // IN / OUT toggle
  // ---------------------------------------------------
  toggleBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      toggleBtns.forEach((b) => {
        b.classList.remove("is-active");
        b.setAttribute("aria-checked", "false");
      });
      btn.classList.add("is-active");
      btn.setAttribute("aria-checked", "true");
      state.type = btn.dataset.value;
    });
  });

  // ---------------------------------------------------
  // Geolocation
  // ---------------------------------------------------
  locateBtn.addEventListener("click", () => {
    if (!("geolocation" in navigator)) {
      locationNote.textContent = "This device doesn't support location.";
      return;
    }

    locateBtn.disabled = true;
    locationNote.textContent = "Locating…";

    // Take one quick reading, but keep listening for a few more seconds
    // to see if a more accurate (lower "accuracy" number = better) fix
    // comes in — GPS often improves after the first second or two, and
    // this avoids settling for a rough WiFi/network-based guess.
    const REFINE_WINDOW_MS = 6000;
    const GOOD_ENOUGH_ACCURACY_M = 20;

    let best = null;
    let watchId = null;
    let finished = false;

    function useBestAndFinish() {
      if (finished) return;
      finished = true;
      if (watchId !== null) navigator.geolocation.clearWatch(watchId);

      if (best) {
        state.coords = {
          latitude: best.coords.latitude,
          longitude: best.coords.longitude,
        };
        locationNote.textContent =
          `Captured: ${best.coords.latitude.toFixed(5)}, ${best.coords.longitude.toFixed(5)} ` +
          `(±${Math.round(best.coords.accuracy)}m)`;
      } else {
        locationNote.textContent = "Couldn't get an accurate location. Try again outdoors or near a window.";
      }
      locateBtn.disabled = false;
      evaluateSubmitEnabled();
    }

    watchId = navigator.geolocation.watchPosition(
      (pos) => {
        if (!best || pos.coords.accuracy < best.coords.accuracy) {
          best = pos;
        }
        // Good enough already — no need to keep waiting
        if (pos.coords.accuracy <= GOOD_ENOUGH_ACCURACY_M) {
          useBestAndFinish();
        }
      },
      (err) => {
        if (finished) return;
        finished = true;
        if (watchId !== null) navigator.geolocation.clearWatch(watchId);
        state.coords = null;
        locationNote.textContent = "Couldn't get location — " + describeGeoError(err);
        locateBtn.disabled = false;
        evaluateSubmitEnabled();
      },
      { enableHighAccuracy: true, timeout: REFINE_WINDOW_MS, maximumAge: 0 }
    );

    setTimeout(useBestAndFinish, REFINE_WINDOW_MS);
  });

  function describeGeoError(err) {
    switch (err.code) {
      case err.PERMISSION_DENIED:
        return "location permission was denied.";
      case err.POSITION_UNAVAILABLE:
        return "position unavailable.";
      case err.TIMEOUT:
        return "it took too long.";
      default:
        return "unknown error.";
    }
  }

  // ---------------------------------------------------
  // Submit gating — depends on which step we're on
  // ---------------------------------------------------
  function evaluateSubmitEnabled() {
    if (state.awaitingPin) {
      submitBtn.disabled = !(navigator.onLine && employeePinInput.value.trim().length > 0);
    } else {
      submitBtn.disabled = !(
        navigator.onLine &&
        employeeIdInput.value.trim().length > 0 &&
        !!state.coords
      );
    }
  }
  employeeIdInput.addEventListener("input", evaluateSubmitEnabled);

  // ---------------------------------------------------
  // Two-step Submit
  // Step 1: user has filled ID + location -> clicking
  //         Submit reveals the PIN field instead of
  //         sending anything yet.
  // Step 2: user enters PIN -> clicking Submit (now
  //         labeled to confirm) actually sends the request.
  // ---------------------------------------------------
  submitBtn.addEventListener("click", async () => {
    if (!state.awaitingPin) {
      goToPinStep();
      return;
    }

    const id = employeeIdInput.value.trim();
    const pin = employeePinInput.value.trim();
    if (!id || !pin || !state.coords) return;

    submitBtn.disabled = true;
    submitBtn.classList.add("is-loading");
    hideResult();

    try {
      const data = await apiRequest({
        action: "submit",
        employeeId: id,
        pin: pin,
        deviceId: deviceId,
        deviceInfo: deviceInfo,
        type: state.type,
        latitude: state.coords.latitude,
        longitude: state.coords.longitude,
      });

      if (data && data.success) {
        showResult(true, data);
        resetToDetailsStep(); // clears PIN and goes back to a fresh form
      } else {
        setNote(pinNote, (data && data.message) || "Incorrect PIN.", "bad");
        employeePinInput.value = "";
        employeePinInput.focus();
        showResult(false, data);
      }
    } catch (err) {
      showResult(false, { message: err.message });
    } finally {
      submitBtn.classList.remove("is-loading");
      evaluateSubmitEnabled();
    }
  });

  function goToPinStep() {
    state.awaitingPin = true;
    pinField.hidden = false;
    backBtn.hidden = false;
    submitBtn.textContent = "Confirm";
    employeeIdInput.disabled = true;
    toggleBtns.forEach((b) => (b.disabled = true));
    locateBtn.disabled = true;
    setNote(pinNote, "", null);
    employeePinInput.value = "";
    employeePinInput.focus();
    evaluateSubmitEnabled();
  }

  function resetToDetailsStep() {
    state.awaitingPin = false;
    pinField.hidden = true;
    backBtn.hidden = true;
    submitBtn.textContent = "Submit";
    employeeIdInput.disabled = false;
    toggleBtns.forEach((b) => (b.disabled = false));
    locateBtn.disabled = false;
    employeePinInput.value = "";
    setNote(pinNote, "", null);
  }

  backBtn.addEventListener("click", () => {
    resetToDetailsStep();
    evaluateSubmitEnabled();
  });

  employeePinInput.addEventListener("input", evaluateSubmitEnabled);

  function hideResult() {
    resultBox.hidden = true;
    resultBox.innerHTML = "";
  }

  function showResult(success, data) {
    resultBox.hidden = false;
    resultBox.classList.toggle("is-good", success);
    resultBox.classList.toggle("is-bad", !success);

    if (success) {
      resultBox.innerHTML = `
        <p class="result-title">✓ ${escapeHtml(data.message || "Attendance submitted.")}</p>
        <dl>
          <dt>Employee</dt><dd>${escapeHtml(data.employee || "—")}</dd>
          <dt>Department</dt><dd>${escapeHtml(data.department || "—")}</dd>
          <dt>Type</dt><dd>${escapeHtml(state.type)}</dd>
          <dt>Location</dt><dd><a href="${escapeHtml(data.mapsLink || "#")}" target="_blank" rel="noopener">${data.latitude?.toFixed(5)}, ${data.longitude?.toFixed(5)}</a></dd>
        </dl>
      `;
    } else {
      resultBox.innerHTML = `<p class="result-title">✕ Couldn't submit</p><p>${escapeHtml(data && data.message ? data.message : "Something went wrong.")}</p>`;
    }
  }

  function escapeHtml(str) {
    return String(str)
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
  }

  // ---------------------------------------------------
  // Install prompt (Android/desktop Chrome)
  // ---------------------------------------------------
  let deferredPrompt = null;
  window.addEventListener("beforeinstallprompt", (e) => {
    e.preventDefault();
    deferredPrompt = e;
    installBtn.hidden = false;
  });
  installBtn.addEventListener("click", async () => {
    if (!deferredPrompt) return;
    installBtn.hidden = true;
    deferredPrompt.prompt();
    await deferredPrompt.userChoice;
    deferredPrompt = null;
  });
  window.addEventListener("appinstalled", () => {
    installBtn.hidden = true;
  });

  // ---------------------------------------------------
  // Service worker registration
  // ---------------------------------------------------
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
      navigator.serviceWorker.register("service-worker.js").catch(() => {
        // Non-fatal — app still works online without the SW.
      });
    });
  }
})();
