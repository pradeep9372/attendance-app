(function () {
  ...
  // ---------------------------------------------------
  let jsonpCounter = 0;
  function jsonpRequest(params, timeoutMs, isRetry) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        settled = true;
        cleanup();
        // Apps Script can be slow to "wake up" — silently retry once
        // before bothering the user with an error.
        if (!isRetry) {
          jsonpRequest(params, timeoutMs, true).then(resolve, reject);
        } else {
      ...
      script.onerror = function () {
        ...
        settled = true;
        cleanup();
        // A brief network drop (common on mobile signal) can fail the
        // script load outright, not just time out. Retry once here too,
        // after a short pause to let the connection recover.
        if (!isRetry) {
          setTimeout(() => {
            jsonpRequest(params, timeoutMs, true).then(resolve, reject);
          }, 800);
